...
const RAMPS = {
  ...
  // SAVI ≈ 1.5·NDVI/(NDVI+0.5); cuts are the NDVI cuts pushed through that.
  SAVI: [
    { lt: 0.40, color: '#a50026', label: 'Poor' },
    { lt: 0.46, color: '#d73027', label: 'Low' },
    { lt: 0.51, color: '#f46d43', label: 'Below avg' },
    { lt: 0.56, color: '#fdae61', label: 'Moderate' },
    { lt: 0.61, color: '#a6d96a', label: 'Fair' },
    { lt: 0.65, color: '#66bd63', label: 'Healthy' },
    { lt: Infinity, color: '#1a9850', label: 'Vigorous' },
  ],
          ...
          >
            <Tooltip direction="top" offset={[0, -6]} opacity={0.95}>
              <div className="pixel-tooltip">
                ...
                <div className="pixel-tooltip-row">
                  <span>Soil Moisture (NDMI)</span>
                  <strong>{f.properties.NDMI.toFixed(3)}</strong>
                </div>
                <div className="pixel-tooltip-row">
                  <span>Canopy Cover (SAVI)</span>
                  <strong>{f.properties.SAVI.toFixed(3)}</strong>
                </div>
              </div>
            </Tooltip>
          </Rectangle>
    ...
    <div>
      <div className="map-header">
        <div>
          ...
          <p className="panel-subtitle">
            ...
            Vegetation Health (NDVI) {pixelGrid.metadata.ndvi_min.toFixed(2)}–{pixelGrid.metadata.ndvi_max.toFixed(2)}
            ...
            Block mean Vegetation Health (NDVI) <strong>{spatialStats.blockMean.toFixed(3)}</strong> —
            young blocks read lower by nature, which is expected, not stress.
          </p>
        </div>
        <div className="metric-selector">
          {[
            { key: 'NDVI', label: 'Vegetation Health (NDVI)' },
            { key: 'SAVI', label: 'Canopy Cover (SAVI)' },
            { key: 'NDMI', label: 'Soil Moisture (NDMI)' },
          ].map(({ key, label }) => (
            <button
              key={key}
      ...
      <div className="spatial-stats">
        ...
        <div className="spatial-stat-legend">
          <span><span className="dot dot-healthy"></span>Healthy vegetation (NDVI ≥ {NDVI_HEALTHY}) — {spatialStats.healthy} pixels</span>
          <span><span className="dot dot-moderate"></span>Establishing ({NDVI_ATTENTION}–{NDVI_HEALTHY}) — {spatialStats.moderate} pixels</span>
          <span><span className="dot dot-stressed"></span>Needs attention (NDVI &lt; {NDVI_ATTENTION}) — {spatialStats.stressed} pixels · {spatialStats.stressArea} m²</span>
        </div>
        {spatialStats.stressed > 0 && (
          <div className="spatial-action">
            <strong>{spatialStats.stressed} pixels below Vegetation Health threshold (NDVI {NDVI_ATTENTION})</strong> — genuinely low for
