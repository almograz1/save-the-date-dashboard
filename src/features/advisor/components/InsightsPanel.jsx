...
function InsightsPanel({ insights, clean }) {
  ...
  findings.push({
    ...
    body: `Correlation of ${irrigation.correlation.toFixed(2)} between Vegetation Health (NDVI) and Soil Moisture (NDMI). ${
  ...
  findings.push({
    ...
    body: `Peak Vegetation Health (NDVI) in ${seasonal.peak.month} (${seasonal.peak.avg.toFixed(3)}), trough in ${seasonal.trough.month} (${seasonal.trough.avg.toFixed(3)}). Amplitude of ${amplitude.toFixed(3)} — date palms grow most in winter and slow down in summer heat. This is normal and expected.`,
  })
