      ...
      if ('Notification' in window) {
        const send = () => {
          new Notification('Date Farm Alert', {
            body: `Vegetation Health (NDVI) ${currentNdvi.toFixed(3)} is below threshold ${threshold.toFixed(2)} - check field conditions.`,
            icon: '/favicon.ico',
          })
        }
  ...
  return (
    <div className={`alert-banner alert-banner-${isAlert ? 'alert' : 'warning'}`}>
      <div className="alert-banner-left">
        ...
        <div>
          <strong>
            {isAlert ? 'Vegetation Health (NDVI) below threshold' : 'Vegetation Health (NDVI) approaching threshold'}
          </strong>
